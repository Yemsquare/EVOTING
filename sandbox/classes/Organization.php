...
class Organization
{
    public function INSERT_ORG($organization) {
        ...
        if($result->num_rows > 0) {
            echo "<div class='alert alert-danger'>Sorry the association you are trying to insert already exists in the database.</div>";
        } else {
            ...
            if($stmt->execute()) {
                echo "<div class='alert alert-success'>Association was inserted successfully.</div>";
            }
            $stmt->free_result();
        }
        $result->free();
